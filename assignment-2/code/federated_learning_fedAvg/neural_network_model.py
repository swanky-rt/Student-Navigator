...
A minimal MLP for multiclass classification with:
- He initialization (bias column included per layer)
- Sigmoid hidden layers, softmax output
- Cross-entropy loss with optional L2 regularization (excluding bias)
- Column-major inputs (X shape: D × N)
"""

import numpy as np

class NeuralNetwork:
    """Simple feedforward NN for multiclass tasks (layer_sizes like [D, H, C])."""

    def __init__(self, layer_sizes, seed=42, lambd=0.0):
        """Initialize weights with He init; store L2 strength `lambd`."""
        np.random.seed(seed)
    ...
    def compute_cost_multiclass(self, AL, Y_onehot):
        """Cross-entropy + L2 (bias excluded)."""
        N = Y_onehot.shape[1]
    ...
    def backward_pass_multiclass(self, acts, X, Y_onehot):
        """Backprop for softmax+CE; returns grads shaped like self.weights."""
        N = X.shape[1]
    ...
    def train_multiclass(self, X, Y_labels, lr=0.1, max_epochs=200, tol=1e-6, verbose=False):
        """
        Train with softmax cross-entropy.
    ...
    def predict_multiclass(self, X):
        """Return argmax class ids from softmax outputs (shape: (N,))."""
        _, acts = self.forward_pass(X, use_softmax_output=True)
        return np.argmax(acts[-1], axis=0)

    def get_params_vector(self):
        """Flatten all weight matrices (bias-inclusive) into a single vector."""
        flats = [W.reshape(-1) for W in self.weights]
        return np.concatenate(flats, axis=0)

    def set_params_vector(self, vec):
        """Load weights from a flattened vector with stored shapes."""
        offset = 0
        for l in range(len(self.weights)):
