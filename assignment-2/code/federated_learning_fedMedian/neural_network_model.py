...
- He init (bias column included)
- Sigmoid hidden layers, softmax output
- CE loss + optional L2 (bias excluded)
...
class NeuralNetwork:
    """Feed-forward NN; layer_sizes like [D, H, C]."""

    def __init__(self, layer_sizes, seed=42, lambd=0.0):
        """Init weights (He); store L2 strength `lambd`."""
        np.random.seed(seed)
    ...
    def train_multiclass(self, X, Y_labels, lr=0.1, max_epochs=200, tol=1e-6, verbose=False):
        """Train with softmax CE. X: D×N, Y_labels in [0, C)."""
        N = Y_labels.shape[0]
        C = self.layer_sizes[-1]

        valid = (Y_labels >= 0) & (Y_labels < C)
        if not np.all(valid):
            X = X[:, valid]; Y_labels = Y_labels[valid]; N = Y_labels.shape[0]
