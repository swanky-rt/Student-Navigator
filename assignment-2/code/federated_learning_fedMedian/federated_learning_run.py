...
def make_round_token_cleaner(labels_lower: List[str]):
    """Cleaner that strips exact label tokens (word-boundary, case-insensitive) from text."""
...
def make_non_iid_splits(train_df: pd.DataFrame, k: int = 5, by_col: str = "job_role", seed: int = 42) -> List[List[int]]:
    """Simple NON-IID split: group by `by_col` then round-robin assign groups to clients (fallback: IID)."""
