...
- Remove label-token leakage (word-boundary, case-insensitive)
- Split train/test with TF–IDF + label encoding
- Create IID / simple NON-IID (by column) client splits
- Build stable test-set row hashes for reproducibility checks
"""
...
def make_round_token_cleaner(labels_lower: List[str]):
    """Cleaner stripping exact label tokens (word-boundary, case-insensitive) from text."""
...
def make_non_iid_splits(train_df: pd.DataFrame, k: int = 5, by_col: str = "job_role", seed: int = 42) -> List[List[int]]:
    """Simple NON-IID: group by `by_col` then round-robin assign groups to clients (fallback: IID)."""
...
def build_test_hashes(test_df: pd.DataFrame) -> Tuple[Set[str], List[str]]:
    """Return (set of stable row hashes, columns used) to confirm test-set identity."""
    ref = test_df.copy()
