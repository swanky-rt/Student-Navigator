...
class ContextAgentClassifier(nn.Module):
    def __init__(self, model_name='all-MiniLM-L6-v2', num_labels=2, hidden_dim=64):
        ...
        # Explicitly forces CPU to avoid macOS MPS/Metal errors
        self.device = torch.device("cpu")
        
        # The Encoder
        self.encoder = SentenceTransformer(model_name, device='cpu')  # We pass device='cpu' to ensure embeddings stay on CPU
    ...
    def forward(self, text_input):
        ...
        # Clone the tensor to detach it from "inference mode".
    ...
    def predict(self, text_input, id2label):
        ...
        self.eval()
        with torch.no_grad():
            # Ensure input is a list to get a 2D batch tensor [1, Dim] otherwise softmax(dim=1) fails on a 1D tensor.
